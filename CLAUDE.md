...
> **Foco atual:** 8° Semestre · **Compiladores** → [8° Semestre/Compiladores/CLAUDE.md](8°%20Semestre/Compiladores/CLAUDE.md) · **PSM** → [8° Semestre/PSM/CLAUDE.md](8°%20Semestre/PSM/CLAUDE.md)
...
**Placar:** 8 semestres · 6 concluídos · 2 em andamento · 2 disciplinas com spec viva própria (Compiladores e PSM).
...
Linguagens que aparecem ao longo dos semestres: C, C++, Java, Python, MATLAB e, no 8°, **MineonScript** (linguagem didática criada em aula de Compiladores). O 8° tem também **PSM**, cujas atividades são em MATLAB e cujo entregável é relatório PDF + código `.m`.
...
| **[8°](8°%20Semestre)** | **Compiladores (MDC168)** · **PSM — Processamento de Sinais Multimídia (ENG149)** | Markdown, MineonScript, Python, MATLAB | 🔄 **Ativo — foco das sessões** |
| [Maratona Fraldinha 2023](Maratona%20Fraldinha%202023) | 14 problemas de maratona | Python | ✅ Concluído |

**Disciplina ativa tem `CLAUDE.md` próprio.** Hoje Compiladores e PSM têm. Ao começar disciplina nova que vire foco de sessão, criar o dela seguindo o mesmo padrão de seções deste arquivo.
...
- **Um arquivo por exercício**, no padrão `<Disciplina>/AulaXX/Exercicio Y.md`. Um arquivo por exercício mantém o diff legível e permite abrir só o que interessa antes da prova. **Exceção: PSM** organiza por atividade (`PSM/Atividade <DD> <Mes> <AA>/`), com enunciado, resolução e códigos na mesma pasta — a unidade de entrega lá é a atividade inteira, não o exercício.
...
- **`git status` mostra ~370 arquivos modificados que ninguém tocou.** É conversão de fim de linha (LF no commit → CRLF na cópia de trabalho), não edição real. Confirmar antes de commitar com `git diff --ignore-cr-at-eol --numstat | awk '$1!=0||$2!=0'` — só o que essa lista imprime mudou de verdade. **Nunca usar `git add -A` neste repo** enquanto isso não for resolvido: sobe um diff de 370 arquivos que esconde a alteração real. Adicionar arquivo por arquivo, ou por pasta. Correção definitiva: criar um `.gitattributes` com `* text=auto` e renormalizar num commit isolado.
- **O agente não consegue apagar arquivos nem pastas** na pasta montada (`Operation not permitted`), e por isso o `git` roda com limitações — comandos que precisam remover `.git/index.lock` avisam e podem falhar. **Commit e push são feitos pela Renata no terminal dela**, não por aqui.
...
- **Disciplinas ativas** → [8° Semestre/Compiladores/CLAUDE.md](8°%20Semestre/Compiladores/CLAUDE.md) · [8° Semestre/PSM/CLAUDE.md](8°%20Semestre/PSM/CLAUDE.md)
